import { ISwRecord, ISwConfig, ISwQryArg, ISwPageParameter, CommonModule } from "../webapp/common";
...
/**
  * 訊息通知
  */
class Crux003Ctrl extends SwMaster1Ctrl {

    constructor(public $http: ng.IHttpService, public $scope: ng.IScope, public $q: ng.IQService, public Config: ISwConfig) {
        super($q, $scope);

        this.QryArgs.Date1 = { Value: this.getToday(-30), Checked: true } as ISwQryArg;

        //初始排序學號反向排序
        this.pageOrderBy('發送時間', true);
...
export function startApp(sitepath, apppath) {
    angular.module("app", [CommonModule, Sys010Module, 'ngSanitize']).constant('Config', {
        SitePath: sitepath,
        AppPath: apppath
    })
    angular.module("app")
