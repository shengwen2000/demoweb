...
namespace WebApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private readonly IdentityOptions _identityOpt;
        ...
        public AccountController(
            ...
            IOptions<ReCAPTCHASetting> recaptcha,
            IOptions<IdentityOptions> identityOpt,
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, EX_Name = model.EX_Name, PhoneNumber =model.PhoneNumber };
        ...
        {
            _logger.LogInformation(4, $"{User.Identity.Name} 登出 ip={HttpContext.Connection.RemoteIpAddress}");
        ...
        public async Task<IActionResult> ExternalLoginCallback(string returnUrl = null, string remoteError = null)
        {
            ...
            if (result.Succeeded)
            {
                var uid = _ctx.UserLogins.Where(x => x.LoginProvider == info.LoginProvider && x.ProviderKey == info.ProviderKey)
                    .Select(x => x.UserId)
                    .First();
                ;
                var uname = _ctx.Users.Where(x => x.Id == uid)
                    .Select(x => x.UserName)
                    .First()
                    ;
                var y = await _signInManager.GetExternalLoginInfoAsync();
                var email = y.Principal.FindFirst(ClaimTypes.Email).Value;
                _logger.LogInformation(5, $"{uname} 透過{info.LoginProvider} {email} 驗證登入 ip={this.HttpContext.Connection.RemoteIpAddress}");
            ...
            else
            {
                ...
                var usrname = info.Principal.FindFirstValue(ClaimTypes.Name);
                if (info.LoginProvider == "Google")
                {
                    var givename = info.Principal.FindFirstValue(ClaimTypes.GivenName);
                    var surname = info.Principal.FindFirstValue(ClaimTypes.Surname);
                    if (givename.IsTrue() && surname.IsTrue())
                    {
                        usrname = $"{surname}{givename}";
                    }
                }

                return View("ExternalLogin", new ExternalLoginViewModel { Email = email, EX_Name = usrname });
            }
        }
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                //recreate confirm token
                await _userManager.GenerateEmailConfirmationTokenAsync(user);
                await _signInManager.SignInAsync(user, false);
                ViewData["Email"] = user.Email;
                ViewData["EX_Name"] = user.EX_Name;
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await _emailSender.SendEmailAsync(model.Email, "忘記密碼",
                   $"請點選連結來重新設定密碼 <a href='{callbackUrl}'>重設密碼</a>");
                return RedirectToAction(nameof(ForgotPasswordConfirmation));
            }
